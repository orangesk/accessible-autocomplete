...
export default class Autocomplete extends Component {
  ...
  handleInputChange(event) {
    ...
    const queryChanged =
      searchOnInteraction || this.state.query.length !== query.length;
    ...
    return (
      <div className={wrapperClasses} onKeyDown={this.handleKeyDown}>
        ...
        <input
          enterkeyhint={
            enterKeyHint ||
            (isIosDevice() && this.hasAutoselect() ? "done" : "")
          }
        ...
        >
          ...
          {showNoOptionsFound && (
            <li
              className={`${optionClass}${optionClassName ? " " + optionClassName : ""} ${optionClass}--no-results`}
            >
              {tNoResults()}
            </li>
          )}
